...
export default function Customers() {
  ...
  return (
    <div className="p-8 bg-[#f0f9f4] min-h-screen font-sans">
      <PageHeader
        title={<span className="text-3xl font-bold text-gray-800">Customers</span>}
      ...
      {showForm && (
        <form className="bg-white/70 backdrop-blur-md p-8 mb-10 rounded-[2rem] shadow-xl border border-white/50 animate-in fade-in zoom-in duration-300">
          <h3 className="text-xl font-bold mb-6 text-gray-700">New Customer Details</h3>
        ...
        <table className="w-full border-separate border-spacing-y-2">
          <thead>
            <tr className="text-[#059669]">
              <th className="p-5 text-sm font-bold uppercase tracking-widest text-left">Customer ID</th>
              <th className="p-5 text-sm font-bold uppercase tracking-widest text-left">Name</th>
              <th className="p-5 text-sm font-bold uppercase tracking-widest text-left">Email</th>
              <th className="p-5 text-sm font-bold uppercase tracking-widest text-left">Phone</th>
              <th className="p-5 text-sm font-bold uppercase tracking-widest text-center">Loyalty</th>
            </tr>
          </thead>

          <tbody className="bg-transparent">
            {customers.map((c) => (
              <tr key={c.customerId} className="group hover:scale-[1.01] transition-transform duration-200">
                <td className="p-5 bg-gray-50 group-hover:bg-green-50 first:rounded-l-2xl last:rounded-r-2xl font-mono font-bold text-gray-500">
                  {c.customerId}
                </td>
                ...
                <td className="p-5 bg-gray-50 group-hover:bg-green-50 first:rounded-l-2xl last:rounded-r-2xl text-center">
                  <span className={`px-5 py-2 rounded-full text-xs font-black shadow-sm
                    ${c.loyalty === 'Gold' ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 text-white' : 
                      c.loyalty === 'Silver' ? 'bg-gradient-to-r from-gray-300 to-gray-500 text-white' : 
